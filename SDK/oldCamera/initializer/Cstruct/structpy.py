...
class tSdkImageResolution(Structure):
    ...
    ("acDescriptio", c_char * 32), # 该分辨率的描述信息。仅预设分辨率时该信息有效。自定义分辨率可忽略该信息
...
class tSdkTrigger(Structure):
    _fields_ = [
    ("iIndex", c_int), #模式索引号
    ("acDescriptio", c_char * 32)] #  该模式的描述信息


class tSdkPackLength(Structure):
    _fields_ = [
    ("iIndex", c_int), #分包大小索引号
    ("acDescriptio", c_char * 32), #对应的描述信息
    ("iPackSize", c_uint)] #
...
class tSdkCameraDevInfo(Structure):
    _fields_ = [
    ("acProductSerie", c_char * 32), # 产品系列
    ("acProductNam", c_char * 32), # 产品名称
    ("acFriendlyNam", c_char * 32), # 产品昵称，用户可自定义改昵称，保存在相机内，用于区分多个相机同时使用,可以用CameraSetFriendlyName接口改变该昵称，设备重启后生效。
    ("acLinkNam", c_char * 32), # 内核符号连接名，内部使用
    ("acDriverVersio", c_char * 32), # 驱动版本
    ("acSensorTyp", c_char * 32), # sensor类型
    ("acPortTyp", c_char * 32), # 接口类型
    ("acS", c_char * 32), # 产品唯一序列号
